  ...
  <div>
    ...
    <tiny-form :inline="true" :style="{ margin: '12px' }" class="form">
      ...
      <tiny-form-item>
        ...
        <tiny-button @click="handleReset">{{ t('operation.reset') }}</tiny-button>
      </tiny-form-item>
    </tiny-form>
    ...
    <div dataSource="a5f6ef4f">
      <tiny-grid :columns="state.columns1" :fetchData="{ api: getTableData }"></tiny-grid>
    </div>
    <div :style="{ width: quotePopWidth }">循环渲染：</div>
...
const state = vue.reactive({
  ...
  columns: [
    {
      type: props.isEdit ? 'selection' : 'index',
      width: '60',
      title: props.isEdit ? '' : '序号'
    },
    {
      field: 'status',
      title: '状态',
      ...
      slots: {
        default: ({ row }) => (
          <div
            ...
            onClick={emit}
          >
            <TinyInput v-model={row.giveamount}></TinyInput>
